...
describe("ChatMarkdown file option chips", () => {
  ...
  it("preserves rejected citations created by over-indented list recovery", () => {
    ...
    expect(malformedHtml).toContain(
      '<li data-replyable-block="true">Bad :codex-file-citation{purpose=&quot;output&quot;}</li>',
    );
...
describe("ChatMarkdown heading levels", () => {
  it("exposes headings below the host heading without changing their tags", () => {
    ...
    // Headings also carry the fork's replyable-block marker, so match the tag
    // and its aria-level rather than the whole element.
    expect(html).toMatch(/<h1[^>]*aria-level="4"[^>]*>Top<\/h1>/);
    expect(html).toMatch(/<h2[^>]*aria-level="5"[^>]*>Section<\/h2>/);
    expect(html).toMatch(/<h6[^>]*aria-level="6"[^>]*>Fine print<\/h6>/);
  });
  ...
  it("leaves heading levels alone when the markdown is not nested", () => {
    ...
    expect(html).toMatch(/<h1(?![^>]*aria-level)[^>]*>Top<\/h1>/);
  });
});
