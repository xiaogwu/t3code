...
export type ComposerSlashCommand = "model" | "plan" | "default" | "rename-thread-ai";
...
export function parseStandaloneComposerSlashCommand(
  ...
  const match = /^\/(plan|default|rename-thread-ai)\s*$/i.exec(text.trim());
  if (!match) {
    return null;
  }
  const command = match[1]?.toLowerCase();
  if (command === "plan") return "plan";
  if (command === "rename-thread-ai") return "rename-thread-ai";
  return "default";
}
