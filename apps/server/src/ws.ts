...
const makeWsRpcLayer = (
  ...
  WsRpcGroup.toLayer(
    Effect.gen(function* () {
      ...
      // The root server layer supplies this shared service. Keep the route
      // layer's public requirements stable for lightweight route tests and
      // alternate embedders by constructing the same service from the
      // already-present dependencies only when an embedder omitted the root
      // layer. Production uses the single root-installed instance.
      const threadTurnBootstrap = yield* Effect.serviceOption(
        ThreadTurnBootstrap.ThreadTurnBootstrap,
      ).pipe(
        Effect.flatMap(
          Option.match({
            onNone: () => ThreadTurnBootstrap.make,
      ...
      const dispatchBootstrapTurnStart = (
        ...
        Effect.gen(function* () {
          ...
          const recordSetupScriptLaunchFailure = (input: {
            ...
            const detail = ThreadTurnBootstrap.projectSetupScriptCompatibilityDetail(input.error);
