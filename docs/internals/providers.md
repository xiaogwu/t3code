...
first, in [`opencoderuntime`][opencoderuntime], on both paths that spawn a server: session start in
`OpenCodeAdapter` and the shared server behind title/commit-message generation.
...
than starting a server whose backend is absent. Two consequences worth knowing:
